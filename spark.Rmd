...
./spark-ec2 -k ec2star -i ~/.ssh/${SSH_KEY}.rsa --region=${REGION} -s ${CLUSTER_SIZE} -w $WAIT_TIME -v ${SPARK_VERSION} launch sparkvm-${YOUR_NAME}
```
...
You can read in each file as an individual observation using `wholeTextFiles()` and you can use `flatMap()` to convert one row into multiple observations.
...
twoKeyValues
# [(u'AVPPIT', 1), (u'LAXIND', 1)]
twoKeyCounts = keyValues.reduceByKey(add).take(2)
# [(u'ORDSEA', 101949), (u'MCIMSY', 925)]
```
...
A standard approach to fitting the Lasso is coordinate descent, as done in R's *glmnet* package, documented here in [Friedman et al.'s well-known (1700 citations according to Google scholar!) paper](http://www.jstatsoft.org/v33/i01) on computation for penalized GLMs.
...
autocorr = 1
rho = 0.7
const = sc.broadcast(autocorr)
rho = sc.broadcast(rho)
...
data = sc.parallelize(xrange(n)).repartition(192).map(simData)
```
...
batches = data.mapPartitions(readPointBatch)
```
...
colMeans = (batchesTmp.map(getSums).reduce(add) / n)[0:P]
...
batches = batches.map(stdizeMean).cache()

def getSumSqs(mat):
    return (mat*mat).sum(axis = 0)

colSds = (pow(batches.map(getSumSqs).reduce(add) / n, 0.5))[0:P]

def stdizeSd(mat):
    mat[:, 0:P] = mat[:, 0:P] / colSds[0:P]
    return mat

batches = batches.map(stdizeSd).cache()
...
Each of the gradient calculation and objective function calculation take about 5 seconds per iteration. 
...
SparkR provides an R interface. It's in development on [Github](http://amplab-extras.github.io/SparkR-pkg/).
...
## 4.4) More resources
  ...
  - The AMPLab will have  dry runs of the AmpCamp talks that you are welcome to attend and give feedback on. Nov. 13, 10 am - 4 pm in Soda 606. Dry runs for the AmpCamp exercises will be Nov. 17, 4 pm - 7 pm, Soda 405. First come first served.


# 5) Homework

1) Suppose we wanted to find the number of departing flights from each airport and add this as a column to our RDD, with the obvious key being the departure airport for each flight. Implement this using Spark. Essentially, of course, you're doing a manual database join. One could then, for example, add the log of the number of flights as a predictor in our regression modeling of flight delays.
...
iteration you solve a least squares problem in close form using the working weights and working observations.
