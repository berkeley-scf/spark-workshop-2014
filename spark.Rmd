...
output = lines.map(createKeyValue).sortByKey().cache()
...
for curkey in keys:
    ...
    fn = '/data/' + curkey
    output.filter(lambda x: curkey == x[0]).map(lambda x: x[1]).repartition(1).saveAsTextFile(fn)
```
...
Let's piece that apart. The mapping step here uses `sample()`, which takes an identifier, $p$, and compute a large number of random vectors, returning the number of vectors whose length is less than one.  So `sample` is the map function
